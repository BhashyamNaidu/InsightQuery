...
## Actual measured results (last run: 2026-09-23, against the live Docker deployment)
...
14 of 15 questions retrieved their expected document at rank 1; one (a question
about the difference between a ward and a community area) retrieved the Chicago Police
Department District Structure document at ranks 1-2 before the expected Glossary of Terms
entry at rank 3 — a defensible near-miss, since that document also discusses wards and
community areas directly, not a retrieval failure. Full per-question detail, including
every retrieved title and rank, is in `docs/rag_eval_results.json`, generated by an actual
run of `scripts/evaluate_rag.py` against the live pgvector database — not a hand-typed
estimate. See "Honest limitations" below for what this result does and doesn't establish.
